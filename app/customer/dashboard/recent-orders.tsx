...
import { useOrders } from "@/features/order/data/orders";

export const RecentOrders = () => {
  const { data, isPending, isError, error } = useOrders({ limit: "5" });

  return (
    <Card className="col-span-6 border shadow-none rounded-2xl ring-0">
      ...
      <CardContent className="px-0 overflow-hidden">
        <Table className="text-base">
          <TableHeader>
            <TableRow className="text-sm font-medium uppercase bg-secondary text-muted-foreground">
            ...
            {(data?.data?.length ?? 0) > 0 ? (
              ...
              <TableRow>
                <TableCell colSpan={4} className="p-0 text-center">
                  {isPending ? (
                    <div className="h-1 rounded-full mb-36 animate-pulse bg-primary"></div>
                  ) : isError ? (
